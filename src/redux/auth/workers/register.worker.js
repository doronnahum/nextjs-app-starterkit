...
function* register(action) {
  const { email, mobile, password, nextRoute /* firstName, lastName */ } = action.payload;
  try {
    yield put(startLoading({ loaderType: LoaderTypes.REGISTER }));
    const response = yield httpRequest(ApiService.createUser, email, mobile, password /* firstName, lastName */);
